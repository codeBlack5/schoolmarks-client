...
export default function Grades() {
  ...
  async function handleDelete(id) {
    if (!confirm("Delete this grade? All its subjects and students will be removed too.")) return;
    try {
    ...
    <div className="max-w-2xl mx-auto p-4 sm:p-6">
      ...
      <div className="overflow-x-auto"><table className="w-full text-sm border border-slate-200 rounded-lg overflow-hidden">
        <thead className="bg-slate-100 text-slate-600 text-left">
          <tr><th className="px-3 py-2">Name</th><th className="px-3 py-2">Level</th><th className="px-3 py-2"></th></tr>
        </thead>
        <tbody>
          {grades.map((g) => (
